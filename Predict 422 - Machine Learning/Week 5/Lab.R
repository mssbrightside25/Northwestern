...
fit.6=lm(education~age,data=Wage)
fit.7=lm(education~poly(age,2),data=Wage)
fit.8=lm(education~poly(age,3),data=Wage)
...
fit=lm(wage~bs(age,knots=c(25,40,60,80)),data=Wage)
pred=predict(fit,newdata=list(age=age.grid),se=T)
plot(age,wage,col="gray")
lines(age.grid,pred$fit,lwd=2)
lines(age.grid,pred$fit+2*pred$se,lty="dashed")
